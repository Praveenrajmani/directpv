...
func (client *Client) SyncDevices(ctx context.Context, nodeID directpvtypes.NodeID) error {
	deviceMap, err := device.ProbeDeviceMap()
	if err != nil {
		return err
	}
	...
	for i := range drives {
		if err = retry.RetryOnConflict(retry.DefaultRetry, func() error { return client.updateDrive(ctx, drives[i].Name, deviceMap) }); err != nil {
			return err
		}
	}
	return nil
...
func (client *Client) updateDrive(ctx context.Context, driveName string, deviceMap map[string][]device.ProbedDevice) error {
	...
	default:
		...
		client.Eventf(drive, EventTypeWarning, EventReasonDriveHasMultipleMatches, "unable to mount the drive due to %v", err)
...
func syncDrive(drive *types.Drive, device device.ProbedDevice) (updated bool) {
...
func (client *Client) verifyDrive(drive *types.Drive) (updated bool) {
	...
	if err := xfs.Mount(source, target); err != nil {
		...
		client.Eventf(drive, EventTypeWarning, EventReasonDriveMountError, "unable to mount the drive; %v", err)
